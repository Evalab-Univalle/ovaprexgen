...
abstract public class Individual {
    ...
    public Individual(String c, Genome g, int gc) throws Exception {
        ...
        if (g == null) {
            throw new Exception("No puede crear un genoma vacio.");
        }
        if (c == null) {
            throw new Exception("Llama a otro constructor.");
        }
        if (gc <= 0) {
        ...
        if (c.length() % g.getGeneLength() != 0) {
            throw new Exception("Tamaño del cromosoma equivocado");
        }
	...
	g.verGenome();
        if (g == null) {
            throw new Exception("No puede crear un individual con un Genoma nulo.");
        }
        if (gc <= 0) {
            throw new Exception("Error de conteo de genes");
        }
    ...
    public void randomChromosome(Random r) {
        if (genome == null) {
            System.err.println("No puede generar un cromosama desde un genoma nulo.");
            return;
        }
